...
neuronale Netze zu betrachten. Falls Beispielsweise die Interesse besteht, die Lösung einer gewöhnlichen
...
Sei $\mathcal{C}(X)$ die oben genannte Kostenfunktion des neuronalen Netzwerks mit der Ausgabe \linebreak
$\mathcal{C}(X)=Y:=(y_j)_j$, wobei $X$ die Gewichte aller Schichten inklusive Bias sind. Dabei ist $Y$ offensichtlich
abhängig von den Gewichten und den Bias. Unser Ziel ist es $\mathcal{C}(X)$ zu minimieren, weshalb wir an dessen
...
\end{align}
wobei $\mu:[t_0,t_f] \rightarrow \mathbb{R}$ durch $\mu(t) = e^{\lambda (t_0 - t)}$ gegeben ist. Die Batches \textit{B}
...
kann. Deshalb ist es sinnvoll, die exponentiell fallende Korrekturfunktion $\mu$ zu nutzen, um den lokalen Fehler in
...
\]
mit Hilfe einiger stochastischer Umformungen und der Annahme, dass die Aktivierungsfunktion $\Psi(x)=x$ ist, die Gleichung
\[
