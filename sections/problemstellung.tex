...
\begin{satz}[Existenzsatz- und Eindeutigkeitssatz von Picard-Lindelöf]
    ...
    Lipschitz-stetig bzgl. $x$ auf $\cal{G}$. Dann besitzt das Anfangswertproblem \eqref{firstorder-ode} für jedes Paar
    $(t_{0},x_{0}) \in {\cal G}$ genau eine lokale Lösung, d.h., es existiert ein $a=a(t_{0},x_{0}) \geq 0$, sodass
    das Anfangswertproblem \eqref{firstorder-ode} auf dem Intervall $[t_{0}-a,t_{0}+a]$ genau eine Lösung $\hat{x}$ hat.
\end{satz}
