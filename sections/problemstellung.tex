...
In diesem Abschnitt wird die Theorie zu den gewöhnlichen Differentialgleichungen erläutert. Dieser orientiert sich
...
\begin{definition}
    ...
    \begin{align}
        x^{(m)} = f(t, x, x^{\prime}, x^{\prime\prime}, \dots, x^{(m-1)}),\quad x(t_{0})=x_{0,1},\quad
        x^{\prime}(t_{0})=x_{0,2},\dots,x^{(m-1)}(t_{0})=x_{0,m}. \label{eq:2}
    \end{align}
...
\begin{definition}
    \label{steife-dgl}
    Eine lineare Differentialgleicuhng \eqref{linear-first-order-ode} mit $A \in \mathbb{R}^{n \times n}$ und
...
$\left\lVert x \right\rVert_2 = \left( \sum_{i=0}^{n} (x_i)^2  \right)^{\frac{1}{2}}$ beschreiben. Der Beweis des
folgenden Satzes beruht auf Überlegungen von H. Heuser in
...
\]
Betrachte nun die Menge $K := \{ x \in C(D): \left\lVert x(t) - x_0 \right\rVert_2 \leq \beta \text{ für alle } t \in D \}$,
...
\[
    \left\lVert x - x_0 \right\rVert_2 = \left\lVert \lim_{n \rightarrow \infty}(x_n) - x_0 \right\rVert_2
    = \lim_{n \rightarrow \infty} \left\lVert (x_n) - x_0 \right\rVert_2 \leq \lim_{n \rightarrow \infty} \beta = \beta.
\]
...
\begin{align*}
    \left\lVert v(t) - v_0 \right\rVert_2
    &= \left\lVert \lambda x(t) + (1-\lambda)y(t) - \lambda x(t_0) - (1-\lambda)y(t_0) \right\rVert_2 \\
...
\[
    \left\lVert f(t,x) - f(t,y) \right\rVert_2 < \frac{\epsilon}{a} \text{ für } \left\lVert x - y \right\rVert_2 < \delta.
\]
...
es folgt
\[
    \left\lVert (Ax)(t) - (Ay)(t) \right\rVert_2 \leq |t - t_0| \frac{\epsilon}{a} = \epsilon \text{ für jedes } t \in D,
\]
...
\end{definition}
Außerdem benötigen wir für den Beweis noch den Fixpunktsatz von Weissinger. Sowohl Satz als auch Beweis lassen
sich in \cite{harroheuserGewohnlicheDifferentialgleichungen} wiederfinden.
\begin{satz}[Fixpunktsatz von Weissinger]
    Sei $\left( B, \left\lVert . \right\rVert_2 \right) $ ein Banachraum und $U \subset B$ abgeschlossen und nichtleer.
    ...
    \[
        \left\lVert A^{j}u - A^{j}v \right\rVert_2 \leq \alpha_j \left\lVert u - v \right\rVert_2 \quad \text{ für alle }
    ...
    \[
        \left\lVert u - u_n \right\rVert_2 \leq \sum_{j=n}^{\infty} \alpha_j \left\lVert u_1 - u_0 \right\rVert_2.
    \]
\end{satz}
...
%\]
Nun können wir einen grundlegenden Satz in der Theorie für gewöhnliche Differentialgleichungen beweisen, welcher sich
...
\label{picard}
    ...
    \[
            {\cal G}=\{(t,x) \in \mathbb{R} \times \mathbb{R}^{n}: |t-t_{0}| \leq \alpha, \quad
        \left\lVert x-x_{0} \right\rVert_2 \leq\beta, \quad \alpha,\beta \geq 0 \},
    \]
...
für alle $j \in \mathbb{N}$ und $t \in D$ gilt. Die Unendlichkeitsnorm $\left\lVert \cdot \right\rVert_{\infty}$ wurde
bereits in dem Beweis von Satz \eqref{peano} definiert.
\begin{alignat*}{2}
    (j=1):& \qquad \left\lVert (A^j u)(t)- (A^j v)(t) \right\rVert_2 &&\leq
    \underbrace{\left\lVert u_0 - v_0 \right\rVert_2}_{=0}
    ...
    &&\leq \underbrace{\left\lVert u_0 - v_0 \right\rVert_2}_{=0} +
...
Hilfssatz. Die grundsätzlichen Überlegungen und Ergebnisse in diesem Kapitel stammen aus
...
\begin{lemma}
    ...
    \begin{align*}
        \left\lVert \hat{x}_a^{\prime}(t) - f(t,\hat{x}_a(t)) \right\rVert_2 &\leq d_e \qquad t \in D,\\
        |t_{0} - \tilde{t_0}| &\leq d_t,\\
        \left\lVert x_0 - \hat{x}_a(\tilde{t_0}) \right\rVert_2 &\leq d_a\\
    \end{align*}
