...
hauptsächlich an den Arbeiten \cite{deuflhardNumerischeMathematikGewohnliche2013,
hairerSolvingOrdinaryDifferential,aulbachGewohnlicheDifferentialgleichungen2004}.
Dieses Kapitel ist die Grundlage für das Verständnis der Schlussfolgerungen und Ergebnisse dieser Arbeit.
...
\begin{alignat}{3}
    \label{autonom-ode-transform}
    z^{\prime} &= 1, \qquad &z(t_0) &= t_0 \nonumber\\
    x^{\prime} &= f(z,x),\qquad  &x(t_0) &= x_0.
\end{alignat}
Die Lösung von dem Anfangswertproblem \eqref{autonom-ode-transform} hat also die Form
...
Lösung ausreicht. Dazu benötigen wir noch einen Satz aus der Funktionalanalysis, welcher,  aus der Literatur
\cite[13, 14]{sundermeierFixpunktsatzSchauder} stammt.
...
beschrieben. Der Beweis des folgenden Satzes beruht auf dem Buch \cite{heuserGewohnlicheDifferentialgleichungen2009} und
wird hier etwas genauer ausgeführt.
...
\label{peano}
    ...
    \]
\end{satz}
$Beweis.$ Sei $J\subset D$ abgeschlossen mit $t_0 \in J$ und $\hat{x}$ eine Lösung von \eqref{firstorder-ode}.
...
differenzierbar ist. Dies bedeutet, dass eine auf $J$ stetige Funktion $\hat{x}(t)$ genau dann das
Anfangswertproblem \eqref{firstorder-ode} löst, wenn es die obige Integralgleichung erfüllt. Für eine auf $J$ stetige
...
Arzela-Ascoli \cite[49]{beckGewohnlicheDifferentialgleichungen2016} hat also jede Folge $(Ax)_n\subset AK$ eine
...
\end{definition}
Eine ähnliche Definition findet sich in \cite[36-37]{hairerSolvingOrdinaryDifferential}. Außerdem
benötigen wir für den Beweis noch den Fixpunktsatz von Weissinger aus
\cite{heuserGewohnlicheDifferentialgleichungen2009}.
\begin{satz}[Fixpunktsatz von Weissinger]
    ...
    \]
\end{satz}
$Beweis.$ \cite[139]{heuserGewohnlicheDifferentialgleichungen2009}\\
%Es gilt
%\[
...
\cite{heuserGewohnlicheDifferentialgleichungen2009} beruht. Wir werden in dieser Arbeit die gegebene Beweisidee etwas
genauer ausführen.
\begin{satz}[Existenz- und Eindeutigkeitssatz von Picard-Lindelöf]
\label{picard}
...
\cite{Matrixexponential} angegeben werden:
\begin{align}
...
\cite{beckGewohnlicheDifferentialgleichungen2016}.
...
\cite[Satz 3.21]{beckGewohnlicheDifferentialgleichungen2016} zeigen, dass $D \subset \hat{D}_{\text{max}}$ gilt.
Demnach haben wir die Fehlerabschätzung $\left\lVert x(t) - \hat{x}(t) \right\rVert_2 \leq \epsilon$
...
%\end{satz}
%$Beweis.$ \cite[69,70]{beckGewohnlicheDifferentialgleichungen2016}
%\begin{bem}
